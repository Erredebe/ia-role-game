...
const normalizeInventory = (inventory: any): Item[] =>
  Array.isArray(inventory)
    ? inventory.map((item: any) => normalizeItem(item))
    : [];
...
const applyBoundedDelta = (
  current: number,
  delta: number,
  max: number
): number => Math.max(0, Math.min(max, current + delta));

// Resolve which slot an item should occupy based on type and current equipment.
const resolveEquipSlot = (
  item: Item,
  equipment: Equipment
): keyof Equipment | undefined => {
  switch (item.type) {
    case "weapon":
      return "mainHand";
    case "armor": {
      const name = normalizeName(item.name);
      return name.match(/(casco|yelmo|sombrero|gorro|head|helmet)/)
        ? "head"
        : "body";
    }
    case "accessory":
...
const applyEquipmentUpdates = (
  ...
  for (const [slot, item] of Object.entries(updates)) {
    ...
    if (
      currentEquipped &&
      !inventoryProvided &&
      !inventoryHasItem(inventory, currentEquipped)
    ) {
...
const syncInventoryWithEquipment = (
  inventory: Item[],
  equipment: Equipment
): void => {
...
export const applyStateUpdate = (
  ...
  if (updatedState.character?.hp !== undefined) {
    ...
    newState.character.hp = applyBoundedDelta(
      oldHp,
      delta,
    ...
    if (delta !== 0) {
      logs.push(
        `HP: ${delta > 0 ? "+" : ""}${delta} (${newState.character.hp}/${
          newState.character.maxHp
        })`
      );
    }
  ...
  if (inventoryProvided) {
    if (!stateChangeJustification?.inventory) {
      ...
      logs.push(
        "❌ Cambio de inventario rechazado: sin justificación narrativa explícita"
      );
      // DO NOT apply the inventory change - keep current inventory
    } else {
      newState.character.inventory = normalizeInventory(
        updatedState.character?.inventory
      );
  ...
  if (equipmentUpdate && typeof equipmentUpdate === "object") {
    ...
    applyEquipmentUpdates(
      equipment,
      equipmentUpdate as Equipment,
  ...
  if (
    (inventoryProvided || equipmentProvided) &&
    newState.character.equipment
  ) {
  ...
  if (updatedState.character?.mana !== undefined) {
    ...
    newState.character.mana = applyBoundedDelta(
      newState.character.mana,
      delta,
...
export const equipItem = (
  state: GameState,
  itemId: string
): { newState: GameState; logs: string[]; success: boolean } => {
  const newState = cloneState(state);
  const logs: string[] = [];
  const equipment = ensureEquipment(newState);

  // Find item in inventory
  const inventoryIndex = newState.character.inventory.findIndex(
    (i: Item) => i.id === itemId
  );
  if (inventoryIndex === -1) {
    return {
      newState,
      logs: ["Item no encontrado en inventario."],
      success: false,
    };
  }
...
export const unequipItem = (
  ...
  if (!item) {
    return {
      newState,
      logs: ["No hay nada equipado en ese slot."],
      success: false,
    };
  }
